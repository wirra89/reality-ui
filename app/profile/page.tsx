...
export default function ProfilePage() {
  ...
  const completenessNudge: Record<string, string> = {
    "Goals":        "Add your goals in Settings",
    "Body metrics": "Add height, weight & age in Settings",
    "Period date":  "Set your period date in Settings",
    "Macros":       "Calculate macros in Settings",
  };
  ...
  const PHASE_SUMMARY: Record<string, (n: string) => string> = {
    menstrual:  (n) => `Rest is productive this week, ${n}. Your body is working hard — prioritise sleep and gentle movement. 💫`,
    follicular: (n) => `Energy is building, ${n}. A great week to push harder and take on new challenges. 🌱`,
    ovulation:  (n) => `You're at your peak, ${n}. Harness this energy — strength and confidence are on your side. ⚡`,
    luteal:     (n) => `Take it steady, ${n}. Your body needs more recovery this week — honour that. 🍂`,
  };
  ...
  return (
    <div className="min-h-dvh bg-background">
      ...
      <main className="relative mx-auto max-w-app px-4 pt-6 pb-28">
        ...
        <header className="flex items-center justify-between mb-4">
          <div>
            <p className="text-xs text-secondary font-semibold uppercase tracking-widest mb-1">
              {activeTab === "me" ? "Your journey" : "Configure"}
            </p>
        ...
        {/* Tabs */}
        <div className="flex p-1 rounded-2xl mb-4" style={{ background: "rgba(196,138,151,0.08)" }}>
          {(["me", "settings"] as const).map(tab => (
            <button
              key={tab}
              ...
              style={{
                ...
                color:      activeTab === tab ? "#C48A97" : "var(--color-text-dim)",
                boxShadow:  activeTab === tab ? "0 2px 8px rgba(196,138,151,0.18)" : "none",
              }}
            >
              {tab === "me" ? "Me" : "Settings"}
            </button>
        ...
        {activeTab === "me" && (
          <>
            ...
            <div className="bg-surface rounded-2xl p-5 shadow-card mb-3 flex flex-col items-center">
              ...
              <div className="relative mb-3">
                <div
                  ...
                  style={{ background: `conic-gradient(${phaseColor} 0deg 240deg, ${phaseColor}22 240deg 360deg)` }}
                >
              ...
              {/* Member since + streak */}
              <p className="text-dark/35 text-xs font-body mb-3">
                {[
              ...
              {/* Smart phase summary */}
              <div
                className="w-full rounded-xl px-3.5 py-3 text-sm font-body leading-relaxed"
                style={{
              ...
              >
                {PHASE_SUMMARY[phase]?.(firstName) ?? ""}
              </div>

              {/* Completeness nudge → taps to Settings */}
              {!isComplete && firstMissing && (
                <div className="w-full mt-4 pt-4 border-t border-[var(--color-border)]">
                  ...
                  <button
                    ...
                    className="w-full text-xs font-semibold text-center py-1 transition-opacity hover:opacity-70"
            ...
            {weightDelta !== null && (
              <div
                className="flex items-center justify-center gap-2 mb-3 px-4 py-3 rounded-2xl"
                style={{ background: "var(--color-surface)", border: "1px solid var(--color-border)", boxShadow: "var(--shadow-soft)" }}
              >
      ...
      {/* Sticky Save — visible when dirty */}
