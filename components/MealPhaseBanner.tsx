...
export default function MealPhaseBanner({ phaseData, cycleDay }: Props) {
    ...
    <div className="rounded-2xl p-4 mb-3" style={{ background: "linear-gradient(135deg, #2A2330, #3D3248)" }}>
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-white/40 text-xs font-body uppercase tracking-widest">{phase} phase · Day {cycleDay}</p>
      ...
      <div className="flex flex-wrap gap-2 mb-2">
        {msg.tips.map((tip) => (
          <div key={tip.label} className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold"
      ...
      <div className="flex gap-1.5">
        {msgs.map((_, i) => (
          <div key={i} className="h-1 rounded-full transition-all duration-300"
            style={{ width: i === msgIdx ? 16 : 6, background: i === msgIdx ? "rgba(255,255,255,0.55)" : "rgba(255,255,255,0.15)" }} />
        ))}
      </div>
    </div>
  );
